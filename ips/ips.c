...
static pthread_mutex_t Pool_Task_Mutex =
PTHREAD_MUTEX_INITIALIZER;
static pthread_cond_t Got_Task_Condition =
...
void ips_create_image_processing_task_pool()
{
    ...
    pthread_t* threads = (pthread_t*) malloc(number_of_threads * sizeof(*threads));

    for (int i = 0; i < number_of_threads; ++i) {
        // TODO
        if (0 != pthread_create(&threads[i], NULL, ips_thread_process_image_part, (void *) pool)) {
...
int getGX(ips_raw_image_t *input_image, png_uint_32 x, png_uint_32 y, int channel, unsigned int channels)
{
    return ((png_bytep)&(input_image->rows[y - 1][(x - 1) * channels]))[channel] +
